  ...
  group('Given a server package directory,', () {
    ...
    test(
      ...
      () async {
        ...
        // A blocking acquire would hang here until the first runner exits,
        // which for a runner expected to stay up for days is never. The
        // timeout is what would catch that.
        await expectLater(
    ...
    test(
      ...
      () async {
        // Not the scenario the lock guards - two runners are two processes -
        // but worth pinning so nobody reads acquire() as a mutex.
    ...
    test(
      ...
      () async {
        // The lock lives on a file descriptor, so this is the crash path: no
        // cleanup runs, and the lock still has to become available.
        final holder = await _spawnLockHolder(tempDir.path);
        await expectLater(

...
///
/// The stack - Docker provisioning included - lives in the runner; `start`
/// only decides whether to spawn one and then attaches. Driving the runner
/// directly is what exercises the provisioning these tests assert on, without
/// detaching a process the test would then have to hunt down.
///
/// Startup either aborts with exit code 1 at the Docker step, or - when Docker
/// is skipped or succeeds - reaches the intentionally invalid model and
/// settles into the degraded stage, where a runner stays up for a client to
/// fix the project and rebuild. That runner is stopped over its socket, the
/// way `serverpod runner stop` does it.

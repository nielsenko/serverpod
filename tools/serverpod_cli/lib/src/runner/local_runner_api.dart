...
/// [RunnerApi] over the in-process watch session, for surfaces that run inside
/// the runner itself.
///
/// Everything here forwards to something that already existed; the value is
/// that it forwards to it once. [WatchSession] serializes reload, restart and
/// migration calls on its own chain, so overlapping commands from the terminal
/// UI and an agent are ordered rather than interleaved.
class LocalRunnerApi implements InProcessRunnerApi {
  ...
  /// The stack, once there is one.
  ///
  /// Null for the window between binding the attach socket and having a stack
  /// to drive - minutes, when generation, Docker and the first compile all
  /// have to happen first. The socket is bound at the start of that window on
  /// purpose: it is exactly when a developer wants to see output, and exactly
  /// when a failure ends the runner before a stack ever exists.
  _Stack? _stack;

  /// Wires the stack this serves, once it exists.
  ///
  /// Everything that needs one reports [RunnerStartingException] until this is
  /// called; [stop] works throughout, because a start that is going nowhere is
  /// the most likely thing to want to abandon.
  void bindStack({
  ...
  ///
  /// [exitCode] carries what the runner is leaving with on
  /// [RunnerStage.stopping], which is the only way a client that renders the
  /// stack rather than hosting it can report the pod's status.
  ///
  /// The event's running flag follows the stage rather than [isRunning]: the
  /// running stage is announced from inside the server factory, before the
  /// session has been handed the process it is about to return, and nothing
  /// announces again once it has. Read live, a client recovering from a
  /// degraded start was told the server was running and not running in the
  /// same breath, and kept every action disabled.
  ...
  ///
  /// One group, built once and broadcast: [RunnerApi.events] promises that
  /// several clients can listen, and a merge is single-subscription. Built per
  /// access, as it was, a second listener on the same stream threw, and every
  /// access opened another set of subscriptions to the same two sources.
...
///
/// Held together so there is one null check rather than five that could
/// disagree about whether the runner has a stack.
class _Stack {
  _Stack({
...
///
/// This mapping is the point where the two surfaces converged. The terminal UI
/// used to log a description and throw, while MCP built a structured result
/// from the same outcome with a differently worded hint; they drifted. Now both
/// render this.
MigrationResult migrationResultFor(CreateMigrationOutcome outcome) {
  ...
  return MigrationResult(
    ...
    created:
        outcome is CreateMigrationCreated ||
        outcome is CreateMigrationServerClientCreated,
  );
}
...
/// Maps a [CreateMigrationOutcome] to a `(message, isError)` pair.
///
/// The message carries no retry instruction. Whether to say `⇧+M` or
/// `force: true` is the surface's business, and [MigrationResult]
/// `abortedForWarnings` is what tells it a retry is even on the table.

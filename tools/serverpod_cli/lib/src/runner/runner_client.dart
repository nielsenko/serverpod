...
class RunnerClient implements RunnerApi {
  ...
  ///
  /// Null keeps trying forever, which is what an interactive UI wants: a
  /// runner restarted in another terminal is picked back up, and the user can
  /// detach whenever they like. A caller with nobody watching - the plain log
  /// stream a CI job runs - passes a bound, since a runner that was killed
  /// outright announces nothing and would otherwise be waited on until the job
  /// times out.
  final Duration? _reconnectDeadline;

  /// The buffers a renderer reads.
  ///
  /// Filled from the snapshot, then kept current from events.
  ///
  ...
  ///
  /// Never completes without one. The runner is not coming back: it was killed
  /// outright, or it aborted before it could announce that it was stopping.
  ...
  /// Fires whenever the local state is replaced wholesale by a snapshot: on
  /// attach, on reconnect, and when the runner swaps the API behind the socket.
  ///
  /// A renderer that derives anything from this client's scalars has to
  /// recompute it here. A snapshot moves them all at once and emits no event
  /// per field, so following [events] alone leaves a derived value - whether
  /// launching apps can do anything, say - reading the state of a runner that
  ...
  /// response - so a peer whose handshake failed ends here too, and reporting
  /// that as a lost connection would start a second reconnect loop alongside
  /// the retry the caller is already doing.
  ...
  /// Tells the renderer to repaint.
  ///
  /// [StartLogHistory.onChanged] is the one channel for that - the same one the
  /// in-process runner fires - so a renderer does not have to work out which of
  /// two to listen to.

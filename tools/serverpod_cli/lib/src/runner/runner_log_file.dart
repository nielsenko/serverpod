...
class RunnerLogFile {
  ...
  /// Rotate once the file passes this size. One previous generation is kept,
  /// as `runner.log.1`.
  ...
  /// How many lines to hold across a rotation before dropping the oldest. A
  /// bound only a wedged rotation reaches, and unbounded is worse: this runs
  /// for days.
  ...
  void _write(String text) {
    ...
    // A line longer than the whole cap goes to an empty file regardless -
    // rotating for it would rotate forever without ever writing it.
    if (_written > 0 && _written + text.length > maxBytes) {
      _hold(text);
      _rotation = _rotate();
      return;
    }
  ...
  Future<void> close() async {
    // Flushing what a rotation held can fill the new file and start another,
    // so drain the chain rather than awaiting only the first.
    ...
    final sink = _sink;
    _sink = null;
    // Written straight through rather than via [_write]: nothing is left to
    // rotate into once the file is closing.
...
class RunnerLogFileSink implements IOSink {
  ...
  void write(Object? object) {
    // Whole runs between newlines rather than a character at a time: every
    // byte of the pod's and the Flutter apps' output passes through here, and
    // `text[i]` allocates a one-character String for each one.
    final text = '$object';
    var start = 0;

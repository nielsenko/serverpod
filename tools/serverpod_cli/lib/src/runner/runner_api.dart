...
abstract interface class RunnerApi {
  ...
  /// Everything after [snapshot]. Broadcast: several clients can attach.
  Stream<RunnerEvent> get events;

  /// Releases this surface: stops emitting on [events] and closes whatever
  /// backs it. The buffers a snapshot reads stay readable afterwards, so a
  ...
  /// server. A no-op once a server is running.
  ...
  /// Dart Tooling Daemon URIs keyed by launched app id. Apps that have not been
  /// launched are absent; a launched app that has not published its DTD yet
  /// maps to `null`.
  ...
  /// The HTTP URI of the VM service proxy in front of the pod, or `null` before

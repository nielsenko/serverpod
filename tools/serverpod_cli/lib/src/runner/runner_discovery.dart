...
/// connections - a crashed runner leaves its manifest behind, so the file's
...
final class NoRunner extends RunnerResolution {
  ...
  /// there was one. A caller starting a runner overwrites it.
...
final class LiveRunner extends RunnerResolution {
  ...
  /// protocol version. Attaching is fine; the caller should say so once.
...
/// new client meeting an old runner is expected rather than exceptional. There
/// is no negotiation: the client refuses and says how to replace the runner.
...
Future<RunnerResolution> resolveRunner(
  ...
  // The attach socket is the runner's front door, but it is served only from
  // the version that introduced it; fall back to the MCP socket so a runner
  // that predates it is still recognized as live rather than replaced.
  final probePaths = [
...
Future<bool> _isListening(String path, Duration timeout) async {
  ...
  try {
    ...
    // No live listener, a path that does not fit, or no support: in every case

...
///
/// Sent once, on connect, so a UI attaching to a runner that has been up for
/// hours draws the full picture rather than starting from the next event. Its
/// bounds are the log history's own: it is a serialization of what the runner
/// already retains, not a second buffer.
class RunnerSnapshot {
  ...
  ///
  /// Both [RunnerApi] implementations answer `snapshot()` from a
  /// [StartLogHistory] and the same five scalars, so the read of the history -
  /// which of its buffers are copied, and how a start time missing from
  /// [StartLogHistory.operationStartTimes] is filled - is stated here once
  /// rather than on each side of the wire.
  factory RunnerSnapshot.from({
    ...
    flutterLines: {
      for (final app in flutterApps)
        app.id: history.flutterLinesFor(app.id).toList(),
    },
  ...
  /// development.
  ///
  /// Decided by the runner: only it knows the run mode the stack was started
  /// with, and a client that guesses offers a key that silently does nothing.
  ...
  ///
  /// Carried rather than left to the events: a URL is published once, when the
  /// app comes up, so a client that attached after that - or reattached, which
  /// is a new client with empty buffers - would otherwise have no way to learn
  /// it and would render the app's tab without an address.
...
/// arguments stay in the runner, which is the only process that launches it.
...
///
/// Path parts come back empty: a client never launches an app itself, it asks
/// the runner to.

...
///
/// Writes the manifest as soon as the runner can be reached, rewrites it as
/// the stage moves and whenever a published address changes, and removes it on
/// a graceful shutdown. A crash skips the removal, which is why liveness is a
/// socket probe rather than the file existing.
class RunnerManifestPublisher {
  ...
  /// values through [resolve].
  ///
  /// [RunnerManifest.vmService] is not the only thing that can change, so
  /// this takes a whole-manifest update rather than a URI. Several sources may
  /// be wired; each [resolve] sees the manifest as the previous one left it.
  void republishOn(
    Stream<void> changes,
  ...
  /// file behind.
  ///
  /// For a start that aborts. The caller that spawned this runner is polling
  /// for the manifest, and one that came and went between two polls reads as
  /// a runner that never came up; left at [RunnerStage.stopping] with the exit
  /// code, it reads as what happened. The next runner replaces it.
...
/// `serverpod runner status` can name the project a human would pass to
/// `docker compose -p`.

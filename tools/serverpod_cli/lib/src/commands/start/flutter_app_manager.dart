...
class FlutterAppManager {
  ...
  ///
  /// [onReady], [onStop] and [onLaunchFailed] report the end of a launch; this
  /// reports its start, so a UI can show the app as busy for the minute a cold
  /// Flutter build takes.
  ...
  /// is loaded.
  ///
  /// Starts false in the runner and is armed by [launchAutoLaunchApps] when a
  /// UI first attaches, so a stack an agent brought up with no UI does not
  /// spawn Flutter processes nobody is watching - and still launches them for
  /// the developer who attaches later.
  ...
  /// URLs of running apps, keyed by app id, in the manner of [dtdUris]. A web
  /// app that has not published one yet, and every non-web device, maps to
  /// `null`; stopped apps are omitted.
  ...
  ///
  /// Idempotent: a second call launches nothing new, so attaching a second UI
  /// - or detaching and attaching again - does not restart apps the developer

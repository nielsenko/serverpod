...
/// Flutter devices that build to JavaScript. They have no VM kernel to push,
/// so the DevFS reload pipeline does not apply to them.
const _webDevices = {
  flutterDeviceWebServer,
...
class FlutterProcess {
  ...
  /// Whether the DevFS reload pipeline can actually drive [_device].
  ///
  /// The pipeline pushes a VM kernel (`.dill`) over `_createDevFS` and swaps
  /// it in with `reloadSources`. Web builds have neither: the code is DDC
  /// JavaScript served by flutter_tools' own asset server, and the vm-service
  /// is DWDS, which does not implement the DevFS RPCs. On a web device the
  /// flag is ignored and reload stays on the daemon `app.restart` path.
  @visibleForTesting
  ...
  Future<void> start() async {
    ...
    if (devFsReloadSupported) {
      ...
      log.info(
        'Flutter DevFS reload is not supported on $_device (web); '
        'using the daemon reload instead.',
      );
    }
  ...
  ///
  /// Web devices always take the shutdown path regardless of [shutdownApp]:
  /// their vm-service is DWDS, which lives inside `flutter_tools` rather
  /// than the app, so SIGKILL takes the reattach URI down with it. Surviving
  /// the CLI buys a browser tab that can never be reattached to, only the
  /// orphan `app.stop` exists to prevent.
  Future<int> stop({
    ...
    if (shutdownApp || _webDevices.contains(_device)) {

...
Map<String, Object?> encodeLogEntry(LogEntry entry) => {
  ...
  'scope': {'id': entry.scope.id, 'label': entry.scope.label},
...
///
/// [fallbackScopeLabel] names the scope when the payload carries none, for a
/// caller that knows the source - the pod, or one Flutter app - better than
/// the empty string would say it.
  ...
  return LogEntry(
    ...
    scope: scope is Map
        ? LogScope(
            ...
            label: scope['label'] as String? ?? fallbackScopeLabel,

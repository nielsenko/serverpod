...
void main() {
  ...
  // The bridge has no equivalent for these, so a migrated deployment that kept
  // serving them would keep a second, weaker way in: they authenticate against
  // the legacy tables and hand out legacy session keys the new stack knows
  // nothing about.
  withServerpod(
  ...
  withServerpod(
    ...
    (final sessionBuilder, final endpoints) {
      ...
      test(
        'when calling Firebase authenticate then the legacy endpoint serves it.',
        () async {
          final legacyClient = _createLegacyClient(sessionBuilder);
          ...
          expect(
            ...
            equals(legacy_auth.AuthenticationFailReason.internalError),
            reason:
                'Reaching the legacy endpoint at all is the point; it fails '
                'here only because no Firebase service account is configured.',
          );
        },
      );
      ...
      test(
        ...
        () async {
          ...
          expect(
            ...
            isFalse,
            reason:
                'Turning off blocking must not turn off forwarding for the '
                'endpoints that do have a bridge equivalent.',
          );
        },
      );
    },

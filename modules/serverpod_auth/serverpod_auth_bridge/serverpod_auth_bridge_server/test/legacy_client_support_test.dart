...
/// entire legacy endpoint surface, but the bridge only forwards `email`,
/// `status` and `user`. The rest keep authenticating against the legacy tables and handing
/// out legacy session keys, so on a deployment that believes it has migrated
/// they are a second, weaker way in.
///
/// The invariant: an endpoint is either forwarded to the bridge or refused -
/// never quietly served by the legacy module.
void main() {
  group('Given legacy client support with the default settings, ', () {
    ...
    for (final endpoint in const [
      ...
      test('when $endpoint is called, then it is blocked.', () {
        expect(
          ...
          reason:
              '$endpoint has no bridge equivalent, so serving it leaves a way '
              'to obtain a legacy session that the new stack knows nothing '
              'about.',
        );
      });
    }
  ...
  group('Given blocking is turned off for a migration in progress, ', () {
    LegacyEndpointDisposition disposition(final String endpoint) =>
